function renderSlide(slide, index) {
  ...
  switch (slide.type) {
    ...
    case 'timeline': return timelineSlide(id, slide, blobs)
    case 'team': return teamSlide(id, slide, blobs)
...
function textSlide(id, s, blobs) {
  return `<div class="slide${s._first ? ' active' : ''}" id="${id}">
    ...
    <div class="slide-content">
      ...
      <h2>${gradify(s.title || '')}</h2>
      ${s.subtitle ? `<div class="sub">${s.subtitle}</div>` : ''}
      ${s.body ? `<div class="body-text">${s.body}</div>` : ''}
    </div>
  </div>`
}
...
function imageSlide(id, s, blobs) {
  return `<div class="slide" id="${id}">
    ...
    <div class="slide-content">
      ...
      <h2>${gradify(s.title || '')}</h2>
      ${s.subtitle ? `<div class="sub">${s.subtitle}</div>` : ''}
      ${s.image ? `<div class="img-wrap"><img src="${s.image}" style="max-width:100%;max-height:50vh;border-radius:16px;border:1px solid var(--border)" /></div>` : ''}
    </div>
  </div>`
}

function timelineSlide(id, s, blobs) {
  const items = (s.timeline || []).map((item, i) => `
    <div class="tl-node${item.done ? ' done' : ''}">
      <div class="tl-marker">
        <div class="tl-dot">${item.done ? '✓' : (i + 1)}</div>
...
function teamSlide(id, s, blobs) {
  const members = (s.team || []).map(m => `
    <div class="team-card">
      ${m.avatar ? `<img class="team-avatar" src="${m.avatar}" alt="${m.name}" onerror="this.style.display='none'">` : `<div class="team-avatar-placeholder">${(m.name || '?')[0]}</div>`}
      <div class="team-name">${m.name}</div>
      <div class="team-role">${m.role || ''}</div>
      ${m.note ? `<div class="team-note">${m.note}</div>` : ''}
    </div>`).join('')
