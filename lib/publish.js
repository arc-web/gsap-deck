...
async function publishToRepo(htmlPath, repo) {
  ...
  // Copy HTML to docs/presentation/
  const destDir = path.join(tmpDir, 'docs', 'presentation')
  fs.mkdirSync(destDir, { recursive: true })
  fs.copyFileSync(htmlPath, path.join(destDir, 'index.html'))

  // Commit and push
  run(`cd "${tmpDir}" && git add -A && git diff --cached --quiet || git commit -m "docs: add presentation" && git push`)
...
async function publishCustom(htmlPath, method, dest) {
  switch (method) {
    ...
    case 'cloudflare': {
      const tmpDir = path.join(CACHE_DIR, '_cf_deploy')
      if (fs.existsSync(tmpDir)) fs.rmSync(tmpDir, { recursive: true, force: true })
      fs.mkdirSync(tmpDir, { recursive: true })
      fs.copyFileSync(htmlPath, path.join(tmpDir, 'index.html'))
      run(`cd "${tmpDir}" && wrangler pages deploy .`)
      fs.rmSync(tmpDir, { recursive: true, force: true })
      break
    }
    default:
